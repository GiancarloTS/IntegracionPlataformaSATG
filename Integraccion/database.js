...
db.serialize(() => {
  ...
  db.run(
    ...
    (err) => {
      if (err)
        console.error(
          "Error al agregar la columna imagen a la tabla productos:",
          err.message
        );
    }
  );
  ...
  db.run(
    ...
    (err) => {
      if (err)
        console.error("Error al crear la tabla sucursales:", err.message);
    }
  );
  ...
  db.run(
    ...
    (err) => {
      if (err)
        console.error("Error al crear la tabla detalle_pedido:", err.message);
    }
  );

  // Crear tabla temporal para carritos pendientes de pago
  db.run(
    `
    CREATE TABLE IF NOT EXISTS carrito_pendiente (
      ...
      fecha_creacion TEXT,
      pref_id TEXT,
      order_id TEXT -- Agregado aquí directamente
    )
    `,
    (err) => {
      if (err)
        console.error(
          "Error al crear la tabla carrito_pendiente:",
          err.message
        );
    }
  );
      ...
      if (err) {
        ...
        const sucursales = [
          ...
          },
          {
            ...
            direccion: "Calle Norte 456, Huechuraba",
          },
    ...
    db.get("SELECT COUNT(*) AS count FROM usuario", (err, row) => {
      if (err) {
        ...
        usuarios.forEach((usuario) => {
          db.run(
            insertQuery,
            [usuario.username, usuario.password, usuario.rol],
            (err) => {
    ...
    db.get("SELECT COUNT(*) AS count FROM inventario", (err, row) => {
      if (err) {
        ...
        inventario.forEach((item) => {
          db.run(
            insertQuery,
            [item.producto_id, item.sucursal_id, item.cantidad],
            (err) => {
              if (err)
                console.error("Error al insertar inventario:", err.message);
            }
          );
        });
    ...
    db.get("SELECT COUNT(*) AS count FROM pedidos", (err, row) => {
      if (err) {
        ...
        pedidos.forEach((pedido) => {
          db.run(
            insertQuery,
            [pedido.cliente_id, pedido.fecha, pedido.total],
            (err) => {
      ...
      if (err) {
        ...
        const detalles = [
          ...
          },
          {
            ...
            cantidad: 2,
            valor_producto: 13000,
            subtotal: 26000,
          },
          {
            pedido_id: 2,
            producto_id: 2,
            nombre_producto: "Taladro Eléctrico",
            cantidad: 3,
            valor_producto: 19000,
            subtotal: 57000,
          },
          {
            pedido_id: 2,
            ...
            cantidad: 1,
            valor_producto: 18000,
            subtotal: 18000,
          },
          {
            pedido_id: 3,
            producto_id: 5,
            nombre_producto: "Cemento Portland",
            cantidad: 3,
        ...
        detalles.forEach((detalle) => {
          db.run(
            ...
            (err) => {
              if (err)
                console.error("Error al insertar detalle_pedido:", err.message);
            }
          );
        });
